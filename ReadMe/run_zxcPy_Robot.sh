...
logfile_Wx_API=$dirPath/Logs/myWeixin_API.log
file_Wx=$dirPath_srcPy/zxcPy.Weixin/zxcPy.Weixin/myWeixin_ItChat.py
file_Wx_API=$dirPath_srcPy/zxcPy.Weixin/myWeixin_API.py
...
count_Wx_API=`ps -ef |grep $file_Wx_API |grep -v "grep" |wc -l`
if [ 0 == $count_Wx_API ];then
	nohup python $file_Wx_API  > $logfile_Wx_API 2>&1 &
echo $count_Wx_API
fi
sleep 10s
