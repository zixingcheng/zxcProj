...
namespace zxcCore.zxcData.DataAnalysis
{
    ...
    /// </summary>
    /// 
    public class DataAnalyse_KeyPoints
    {
        #region 属性及构造
        ...
        public DataAnalyse_KeyPoints(string tag)
        {
            Tag = tag;
        ...
        protected virtual bool dataHandle(double value, DateTime time, bool recursion = false)
        {
            ...
            //计算变化区间
            typeKeyPoints monitorType = typeKeyPoints.NONE;
            typeKeyPoints_detail monitorType2 = typeKeyPoints_detail.NONE;
            ...
            if (dRatio * this.Value_last_direction >= 0)    //同方向，超限触发超限点及该值
            {
                if (dRatio > 0)     //上升,新高       
                {
                    monitorType = typeKeyPoints.RAISE; monitorType2 = typeKeyPoints_detail.RAISE; this.Value_last_direction = 1;
                }
                else               //下降，新低
                {
                    monitorType = typeKeyPoints.FALL; monitorType2 = typeKeyPoints_detail.FALL; this.Value_last_direction = -1;
                }
                ...
                if (Math.Abs(dRatio) < this.Value_delta)
                {
                    ...
                    {
                        //标识波动
                        monitorType2 = this.Value_last_direction < 0 ? typeKeyPoints_detail.RAISE_WAVE : typeKeyPoints_detail.RAISE_WAVE;
                    }
                }
                else
                {
                ...
                if (recursion)
                {
                    monitorType = typeKeyPoints.NONE;
                }
            ...
            else
            {
                ...
                if (Math.Abs(ratioLimit) > this.Value_delta)
                {
                    ...
                    monitorType = typeKeyPoints.BREAK; bHitLimit = true; this.Value_last_direction *= -1;
                    monitorType2 = this.Value_last_direction < 0 ? typeKeyPoints_detail.FALL_BREAK : typeKeyPoints_detail.RAISE_BREAK;
                }
            }

            //组装消息
            if (monitorType != typeKeyPoints.NONE)
            {
                ...
                {
                    DataAnalyse_KeyPoints_EventArgs pArgs = new DataAnalyse_KeyPoints_EventArgs();
