...
namespace zxcCore.zxcRobot.Test
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            _dbTest.Data_Test.Add(new Data_TestObj() { Id = 0, Id_str = "0" });
            _dbTest.Data_Test.SaveChanges();


            Data_Table<Data_TestObj> dbTest = new Data_Table<Data_TestObj>();
            for (int i = 0; i < 100000; i++)
            {
                Data_TestObj obj = new Data_TestObj();
                obj.Id = i;
            ...
            //反序列化
            Data_Table<Data_TestObj> dbTest2 = Newtonsoft.Json.JsonConvert.DeserializeObject<Data_Table<Data_TestObj>>(strJson);
