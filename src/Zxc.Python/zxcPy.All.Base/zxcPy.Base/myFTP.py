...
class myFTP:
    def __init__(self, host, port, localFolder):
        ...
        logDir = os.path.abspath(os.path.join(localFolder, ".."))  
        self.logFile = logDir + '/Logs/log_{}.txt'.format(datetime.datetime.strftime(datetime.datetime.now(), "%Y_%m_%d"))
    ...
    def delete_files(self, days = -5):
        file_list = [self.localFolder]                  # 文件夹列表
...
class myFTP_Monitor:
    ...
    def upLoad_files(self):
        try:
            ...
            # 上传目录
            self.my_ftp.upload_file_tree(self.localDir, self.dataFolder)
