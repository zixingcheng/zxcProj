...
class myObj_Bill():
    ...
    def ToString(self, nSpace = 0, isSimple = False): 
        if(isSimple == False):
            ...
            if(self.usrMoney > 0):
                if(self.usrBillType == myTradeType.投资回笼):
...
class myObj_Bills():
    ...
    def Static(self, usrSrc = '', usrBillType = "", startTime = '', endTime = '', nMonth = 1): 
        ...
        #统计项目初始
        lstSum = {}
        for x in myTradeType: lstSum[x] = 0
        ...
        dSum_In = 0 
        for x in lstBill:
            if(x.usrBillType == myTradeType.投资回笼):       #投资收益单独计算
                ...
                if(startTime > bill.usrTime):              #投资在当前时间前，累加投资项，否则计算有误
                    lstSum[myTradeType.投资] += bill.usrMoney
            else:
        ...
        dSum_Out -= lstSum[myTradeType.投资]       #剔除投资(投资记负但非消费)
        dSum_Out_投资 = lstSum[myTradeType.投资] - dSum_In_投资
        
        #输出信息
        strPerfix = "\n" + " " * 4
        strOut = "账单统计(" + self.usrID + ")："
        strOut += "\n" + "总资产：" + str(round(dSum_Out + dSum_In, 2)) + "元"
        if(lstSum[myTradeType.红包] > 0):
            strOut += strPerfix + "红包收入：" + str(round(lstSum[myTradeType.红包], 2)) + "元"
        if(lstSum[myTradeType.投资回笼] > 0):
            strOut += strPerfix + "投资收益：" + str(round(lstSum[myTradeType.投资回笼], 2)) + "元"
            if(dSum_In_投资 < 0):
        ...
        if(dSum_Out < 0):
            ...
            #消费细分：
            for x in keys:
                if(lstSum[x] < 0 and x != myTradeType.投资):
