...
class myData_Table(myData_DB.myData_Table):
    ...
    def _Init_DB(self, path): 
        ...
        self.pySql = myPyMysql.myPyMysql(usrName='zpkj', usrPw='Zp.666888!@#', dbName = self.nameDB, charset='utf8', host = '120.197.152.99', port = 8606)   #远程操作测试
    ...
    def Add_Row(self, rowInfo = {}, updata = False, bSave = True, tableName = ""):
        ...
        #新增 
        rowInfo['ID'] = self._Get_ID(tableName)
    ...
    def Query(self, fliters, sortField="", reverse=False, tableName = "", pageInd = 1, perSize = 10): 
        ...
        return self.pySql._Query(fliters, tableName = tableName, orderField = sortField, pageInd = pageInd, perSize = perSize) 
    # 查询筛选 --重写
    def _Query(self, fliter, tableName = "", pageInd = 1, perSize = 10): 
        return self.pySql._Query(fliter, tableName = tableName, pageInd = pageInd, perSize = perSize) 
    # 提取与指定筛选条件相同项 --重写
    def _Find_ByFliter(self, field, fliter, data = None, tableName = "", pageInd = 1, perSize = 10): 
        ...
        return self.pySql._Query(strFliter, tableName = tableName, pageInd = pageInd, perSize = perSize) 
    # 提取当前编号行数据 --重写
    ...
    def _Get_ID(self, tableName = ""):
        fliter = F"select max(ID) as maxid from {tableName} where isDel=False;"
    ...
    def _Check(self, rowInfo, updata = False, tableName = ""): 
        ...
        if(rs[0]):
            # 修改
            if(updata):
                self._Updata(rs[1], rowInfo, tableName=tableName)
                return rs[1];
            return 0
        return "" 
    #单条有效修正 --重写
