    ...
    def _Update(self, id, rowInfo = {}, tableName = "", idField = 'ID'):
        ...
        for x in keys:
            ...
            if(type(rowInfo[x]) == str):
                ...
                editInfo += f",{x}='{myData_Trans.Tran_ToDatetime_str(rowInfo[x])}'"
            else:
                editInfo += f",{x}={rowInfo[x]}"

        # 调用修改数据
        editInfo = myData.iif(len(editInfo) > 1, editInfo[1:], editInfo)
        idInfo = myData.iif(type(id) == str, f"'{id}'", id)
        rs = self.execute(self._dbName, f"UPDATE {tableName} SET {editInfo} WHERE {idField} = {idInfo};")
        return not rs == None
    # 删除行数据，必须指定id
    def _Query(self, fliter, tableName = "", fields = [], orderField = '', pageInd = 1, perSize = 0): 
        # 组装查询返回字段
        infoFields = ""
        for x in fields:
            infoFields += ',' + x
        ...
        rs = self.query(f"SELECT {infoFields} FROM {tableName} WHERE {fliter} {orderField} {orderLimit};")
        return rs
    # 删除行数据，必须指定id
